...
export async function GetSetting(user_id: number | null) {
  try {
    const response = await axios.get(`${domain}/api/v1/options/${user_id}`);
...
export async function PutSetting(
  ...
  try {
    ...
    const response = await axios.put(`${domain}/api/v1/options`, jsonFrom, {
      headers: {
...
export async function PutProfile(updatedEmail, updatedPassword, updatedName) {
  try {
    ...
    const response = await axios.put(`${domain}/api/v1/profile`, jsonForm, {
      headers: {
        'Content-Type': 'application/json',
      },
    });
